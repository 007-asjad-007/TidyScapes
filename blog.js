document.addEventListener("DOMContentLoaded", async function () {
    ...
    const recentBlogsContainer = document.querySelector(".recent-blogs-container");
    ...
    try {
      ...
      if (blogDoc.exists) {
        ...
        // Main blog content
        blogContentContainer.querySelector(".blog-title").innerHTML = `${blog.title || "Untitled Blog"}`;
        blogContentContainer.querySelector(".blog-image").src = `${blog.imageURL || "default-image.jpg"}`;
        ...
        blogContentContainer.querySelector(".blog-body").innerHTML = formattedContent;
  
        blogContentContainer.querySelector(".blog-author-date").innerHTML = `By ${blog.author || "Unknown Author"} | ${
          blog.date
      ...
      // Fetch recent blogs
      const snapshot = await db.collection("blogs").orderBy("date", "desc").limit(5).get();
